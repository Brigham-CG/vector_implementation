...
class Interval {
 public:
  ...
  Interval& operator=(const Interval& lhs);  // copy a interval

  float& begin();
  float begin() const; //obtain a begin

  float& end();
  float end() const; // obtain a end

  float get_range(); // end - begin 
...
template <size_t N> // cant of dimentions 
class Rectangle {
 public:
  ...
  Interval operator[](size_t index) const; // return a limit
