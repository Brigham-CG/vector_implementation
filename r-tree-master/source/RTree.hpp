...
class RTree {
 public:
  ...
  struct Node {
    ...
    std::shared_ptr<Node> split_node(const SpatialObject &new_entry);
...
void RTree<N, ElemType, M, m>::Node::pickNext(
              ...
              Rectangle<N>& mbb_g1, Rectangle<N>& mbb_g2) {
  float max_diff = -1.f;
  ...
  for (size_t i = 0; i < entries.size(); ++i) {
    ...
    area_increase_g1 = enlarged_g1.get_area() - mbb_g1.get_area();

    enlarged_g2 = mbb_g2;
    enlarged_g2.adjust(entries.at(i).box);

    area_increase_g2 = enlarged_g2.get_area() - mbb_g2.get_area();
...
RTree<N, ElemType, M, m>::Node::split_node(const SpatialObject &new_entry) {
  ...
  insert(entries.at(first));
  std::shared_ptr<Node> n_node = std::make_shared<Node>();
  n_node->insert(entries.at(second));

  Rectangle<N> mbb_g1 = entries.at(first).box;
  Rectangle<N> mbb_g2 = entries.at(second).box;
  ...
  while (!entries.empty()) {
    if (this->size + entries.size() == m) {
      ...
      entries.clear();
    } else if (n_node->size + entries.size() == m) {
      for (auto& e : entries) n_node->insert(e);
      entries.clear();
    } else {
      pickNext(entries, &idx, mbb_g1, mbb_g2);

      enlarged_mbb_g1 = mbb_g1;
      enlarged_mbb_g1.adjust(entries.at(idx).box);
      area_increase_g1 = enlarged_mbb_g1.get_area() - mbb_g1.get_area();

      enlarged_mbb_g2 = mbb_g2;
      enlarged_mbb_g2.adjust(entries.at(idx).box);
      area_increase_g2 = enlarged_mbb_g2.get_area() - mbb_g2.get_area();

      if (area_increase_g1 < area_increase_g2 || 
      (area_increase_g1 == area_increase_g2 && mbb_g1.get_area() < mbb_g2.get_area())
       || (area_increase_g1 == area_increase_g2 && mbb_g1.get_area() == mbb_g2.get_area() && size <= n_node->size)
       ) {
        mbb_g1.adjust(entries.at(idx).box);
        this->insert(entries.at(idx));
      }
      else {
        mbb_g2.adjust(entries.at(idx).box);
        n_node->insert(entries.at(idx));
      }
